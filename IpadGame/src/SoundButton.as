package  
{
	...
	public class SoundButton extends Button
	{
		...
		{
			super(Texture.fromBitmapData(new BitmapData(Assests.getAtlas().getTexture("soundOff").width, Assests.getAtlas().getTexture("soundOff").height, true, 0)));
		...
		{
			// Normal state - image
			mcUnmuteState = new MovieClip(Assests.getAtlas().getTextures("soundOn"), 3);
			Starling.juggler.add(mcUnmuteState);
			this.addChild(mcUnmuteState);
			
			// Selected state - animation
			imageMuteState = new Image(Assests.getAtlas().getTexture("soundOff"));
